...
ViewQuery.prototype.getQuery = function (key, options) {
  var query = Couchbase.ViewQuery.from(options.view.designDoc, options.view.designView);
	...
	if(options.query.beforeIn && options.query.afterIn) {
		// Request an interval [... , ...]
	...
	} else if(options.query.afterEx && options.query.beforeIn) {
		// Request an interval ]... , ...]
		if(options.query.sort === -1) {
	...
	} else if(options.query.afterIn && options.query.beforeEx) {
		// Request an interval [... , ...[
		if(options.query.sort === -1) {
	...
	} else if(options.query.afterEx && options.query.beforeEx) {
		// Request an interval ]... , ...[
		if(options.query.sort === -1) {
	...
	} else if(options.query.beforeIn) {
		// Request an interval ]-∞ , ... ]
		if(options.query.sort === -1) {
	...
	} else if(options.query.beforeEx) {
		// Request an interval ]-∞ , ... [
		if(options.query.sort === -1) {
