...
CouchbaseSmart.prototype._getConnection = function (cb) {
  if(this._db) {
	...
	this._db = cluster.openBucket(this._config.bucket, this._config.password, function (err) {
		return cb(err);
	});
};
/**
 ...
 * @method disconnect
 *
 * @desc Method to close connection to Couchbase
 *
 * @param  {disconnectCallback} cb
 *
 */
CouchbaseSmart.prototype.disconnect = function (cb) {
  this._db.disconnect();
  cb();
};
/**
...
CouchbaseSmart.prototype.insert = function (key, data, cb) {
  ...
  var onConnection = function (err) {
    ...
    var onInsert = function (insertErr) {
      if(insertErr) {
        if(insertErr.code === Couchbase.errors.timedOut) {
          self._db = null;
        }
        return cb(insertErr);
      }
 ...
 *
 * @param  {String}         key
 * @param  {countCallback}  cb
 *
 */
CouchbaseSmart.prototype.count = function (key, cb) {
  if(typeof(key) !== 'string') {
    return cb(new TypeError("Key must be a String"));
  }
